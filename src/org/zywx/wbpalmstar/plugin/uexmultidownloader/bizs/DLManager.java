package org.zywx.wbpalmstar.plugin.uexmultidownloader.bizs;
...
import org.zywx.wbpalmstar.plugin.uexmultidownloader.cons.HttpConnPars;
...
import org.zywx.wbpalmstar.plugin.uexmultidownloader.entities.ThreadInfo;
import org.zywx.wbpalmstar.plugin.uexmultidownloader.interfaces.DLTaskListener;
import org.zywx.wbpalmstar.plugin.uexmultidownloader.interfaces.IDLThreadListener;
import org.zywx.wbpalmstar.plugin.uexmultidownloader.utils.FileUtil;
import org.zywx.wbpalmstar.plugin.uexmultidownloader.utils.NetUtil;
...
public final class DLManager {
    ...
    private static final int NET_SPEED_RATE=1000;//1秒
    ...
    public void dlStart(String url, String filePath, DLTaskListener listener) {
        DLPrepare dlPrepare = new DLPrepare(url, filePath, listener);
    ...
    private class DLPrepare implements Runnable {
        private String url, filePath;// 下载路径和保存地址
        private DLTaskListener listener;// 下载监听器

        private DLPrepare(String url, String filePath, DLTaskListener listener) {
            this.url = url;
        ...
        public void run() {
            ...
            try {
                ...
                if (sTaskDLing.containsKey(url)) {
                    ...
                    if (dlTask!=null) {
                        dlTask.setListener(listener);
                    }
                } else {
                    TaskInfo info = sDBManager.queryTaskInfoByUrl(url);
                    String fileName = FileUtil.getFileNameFromUrl(filePath).replace("/", "");
                    if (null != listener) {
                        listener.onStart(fileName, realUrl);
                    }
                    File file = new File(filePath);
                    if (null == info || !file.exists()) {
                        info = new TaskInfo(FileUtil.createFile(filePath), url, realUrl, 0, 0);
                    }
    ...
    private class DLTask implements Runnable, IDLThreadListener {
        ...
        private int lengthPerSecond=0;
        private boolean isResume;
        private boolean isStop;
        private boolean isExists;
        private boolean isConnect = true;
        private NetSpeedThread speedThread=new NetSpeedThread();
        private List<ThreadInfo> mThreadInfos;
        private boolean callbackNetSpeed=true;
        ...
        private DLTask(TaskInfo info, DLTaskListener listener) {
            ...
            if (null != sDBManager.queryTaskInfoByUrl(info.baseUrl)) {
                ...
                if (null != mThreadInfos && mThreadInfos.size() != 0) {
                    isResume = true;
                } else if (info.progress!=info.length){
                    sDBManager.deleteTaskInfo(info.baseUrl);
                }
            }
        }
                    ...
                    try {
                        ...
                        if (conn.getResponseCode() == HttpStatus.SC_PARTIAL_CONTENT) {
                            ...
                            if (info.dlLocalFile.exists() && info.dlLocalFile.length() == fileLength) {
                                ...
                                if (null != mListener) {
                                    mListener.onFinish(info.dlLocalFile);
                                    mListener.onProgress(100,fileLength,fileLength);
                                    callbackNetSpeed=false;
                                }
                            }
                            if (!isExists) {
        ...
        public void setListener(DLTaskListener mListener) {
        ...
        public void onThreadProgress(int progress) {
            synchronized (this) {
//                Log.i(TAG, "onThreadProgress " + progress);
                lengthPerSecond+=progress;
                totalProgress += progress;
                int tmp = (int) (totalProgress * 1.0 / fileLength * 100);
                if (null != mListener && tmp != totalProgressIn100) {
                    mListener.onProgress(tmp,fileLength,totalProgress);
                    totalProgressIn100 = tmp;
                    if (!callbackNetSpeed&&tmp!=100){
                        callbackNetSpeed=true;
                        speedThread.start();
                    }
                }
...
//                  sDBManager.deleteTaskInfo(info.baseUrl);
                ...
                if (isStop) {
                    ...
                    sTaskDLing.remove(info.baseUrl);
                    callbackNetSpeed=false;
                    handler.removeCallbacks(speedThread);
                }
            }
        }
        ...
        private class NetSpeedThread extends Thread{
            ...
            public void run() {
                if (mListener!=null){
                    mListener.onNetSpeed(1000*lengthPerSecond/1024/NET_SPEED_RATE);
//                    Log.i(TAG,"speed:   "+1000*lengthPerSecond/1024/NET_SPEED_RATE);
                }
                if (callbackNetSpeed) {
                    handler.postDelayed(speedThread, NET_SPEED_RATE);
                }
                lengthPerSecond=0;
            }
